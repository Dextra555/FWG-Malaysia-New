...
export class EmployeeListComponent implements OnInit {
  ...
  onSubmit() {
    ...
    const loginId = this.currentUser ?? '';
    let url = environment.baseReportUrl + this.currentUrl + "LoginID=" + loginId;
    url += "&Branch=" + this.frm.get("branch")?.value;
    url += "&EmployeeType=" + this.frm.get("type")?.value;
    const csgtngStatus = this.frm.get("csgtngStatus")?.value || "All";
    url += "&CSGTNGStatus=" + csgtngStatus;
    this.showLoadingSpinner = true;
