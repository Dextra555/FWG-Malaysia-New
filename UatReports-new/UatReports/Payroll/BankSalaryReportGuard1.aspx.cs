...
string connStr = System.Configuration.ConfigurationManager.ConnectionStrings["obms"]?.ConnectionString
            ...
            if (this.Page.Request.QueryString["Bank"] != "")
                CrystalReportSourceData.ReportDocument.RecordSelectionFormula += " AND {EmployeeSalaryDetails.EMPFL_BANK}= '" + paramBank + "'";
            if (this.Page.Request.QueryString["Branch"] != "")
                CrystalReportSourceData.ReportDocument.RecordSelectionFormula += " AND {BranchMaster.Code}= '" + paramBranch + "'";
