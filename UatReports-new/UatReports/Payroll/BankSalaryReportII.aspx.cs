...
string connStr = System.Configuration.ConfigurationManager.ConnectionStrings["obms"]?.ConnectionString
            ...
            if (this.Page.Request.QueryString["Bank"] != "0")
                CrystalReportSourceData.ReportDocument.RecordSelectionFormula += " AND {vwBankSalaryStatement.EMPFL_BANK}= '" + paramBank + "'";
            if (this.Page.Request.QueryString["Branch"] != "0")
                CrystalReportSourceData.ReportDocument.RecordSelectionFormula += " AND {vwBankSalaryStatement.Code}= '" + paramBranch + "'";

            if (this.Page.Request.QueryString["Branch"] != "0")
                CrystalReportViewerView.ParameterFieldInfo["Branch"].CurrentValues.Add(paramBranch);
            else
                CrystalReportViewerView.ParameterFieldInfo["Branch"].CurrentValues.Add("ALL BRANCHES");
