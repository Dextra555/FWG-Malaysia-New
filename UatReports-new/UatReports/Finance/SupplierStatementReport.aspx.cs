...
public partial class Finance_SupplierStatementReport : System.Web.UI.Page
{
	protected void Page_Load(object sender, EventArgs e)
	{
        try
        {
            ...
            if (DateTime.TryParse(startDate, out periodStartDate))
            {
            ...
            if (DateTime.TryParse(endDate, out periodEndDate))
            {
            ...
            paramStatus.Value = Request.QueryString["Status"];

            ParameterDiscreteValue paramUserName = new ParameterDiscreteValue();
            paramUserName.Value = Request.QueryString["LoginID"];

            ParameterDiscreteValue paramSupplier = new ParameterDiscreteValue();
            paramSupplier.Value = Request.QueryString["Supplier"];
            ...
            CrystalReportViewer.ParameterFieldInfo["Supplier"].CurrentValues.Add(paramSupplier);

            if (Request.QueryString["Branch"] != "0")
            {
                CrystalReportSource.ReportDocument.Subreports[0].RecordSelectionFormula += " AND {command.Branch}='" + Request.QueryString["Branch"] + "'";
            }
