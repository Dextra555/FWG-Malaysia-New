...
public partial class Finance_PaymentSummaryCustomReport : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                //CrystalReportSource.ReportDocument.RecordSelectionFormula += " AND {BankMaster.BankId}=" + Request.QueryString["Bank"];
            }           
            if (Request.QueryString["Branch"] != "0")
            {
                //CrystalReportSource.ReportDocument.RecordSelectionFormula += " AND {BranchPaymentDetails.Branch}='" + Request.QueryString["Branch"] + "'";
            }
            if (Request.QueryString["PaymentType"] != "0")
            {
                //CrystalReportSource.ReportDocument.RecordSelectionFormula += " AND {BranchPayments.PaymentType}=" + Request.QueryString["PaymentType"];
            }
            if (Request.QueryString["Purpose"] != "0")
            {
                //CrystalReportSource.ReportDocument.RecordSelectionFormula += " AND {BranchPayments.PaymentPurpose}=" + Request.QueryString["Purpose"];
